...
本文对照 `docs/sdk/feature-checklist.md`、当前源码和测试结果，记录 V2 已实现、部分实现和未实现的能力。结论：当前项目已经形成“像素上报 SDK + 会话画像 + 行为路径 Worker 聚合 + 可靠推送层 + 富模板测试站点 + 数据后台解析”的可验证闭环，但服务端幂等、真实浏览器回归和更多采集插件仍需继续补齐。
...
| 可靠推送 | 已实现 | 支持 `batchSize` 自动 flush、失败重试、离线缓存和网络恢复补发。 |
...
| 队列 flush | 支持内存队列、手动 flush、配置式 `flushInterval`、`batchSize` 阈值触发和并发 flush 保护。 | 尚未实现服务端批次幂等和多标签队列协调。 |
...
| 命令 | 覆盖 |
| --- | --- |
| `npm test` | 24 个单元测试，覆盖像素上报、初始化画像、生命周期、行为路径、可靠推送、隐私脱敏、点击、表单、路由、API、错误、录屏懒加载、重复初始化和超长诊断。 |
...
1. 补服务端 `batch_id` 幂等和去重，避免网络重试造成重复入库。
