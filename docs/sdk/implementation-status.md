...
本文对照 `docs/sdk/feature-checklist.md`、当前源码和测试结果，记录 V2 已实现、部分实现和未实现的能力。结论：当前项目已经形成“像素上报 SDK + 会话画像 + 行为路径 Worker 聚合 + 可靠推送层 + 服务端幂等去重 + 富模板测试站点 + 数据后台解析”的可验证闭环，但真实浏览器回归和更多采集插件仍需继续补齐。
...
| 服务端幂等 | 已实现 | 像素请求带 `baid` 和 `dk`，mock server 按 `dk` 去重并通过 `/__stats` 暴露丢弃数量。 |
...
| 幂等去重 | `baid`、`dk`、`/__stats` | 重试和重复请求不重复写入 `hits`，后台展示“服务端去重”。 |
